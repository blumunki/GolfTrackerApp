...
Round linking implemented (WORKLOG 3-6): the round-recording setup step offers a "Competition (optional)" selector listing non-terminal competitions the recorder's own player has entered (`GetCompetitionsForPlayerAsync`); the chosen competition is set on the round at creation (both the RecordRound page and the RoundWorkflowService/live path via `RoundWorkflowSession.CompetitionId`). Round details shows the linked competition in the header meta and lets the round creator (or admin) link/unlink an existing round (`AssignRoundAsync`/`UnassignRoundAsync` ownership rules apply). `RoundType` remains untouched by linking.
