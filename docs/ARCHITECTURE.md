...
| 3 | Competitions & Scoring Formats | 🚧 In progress | Core loop, ownership, and public registration complete (3-1 through 3-9, plus 3-11). Historical competition import remains (3-10) |
...
Service layer implemented (WORKLOG 3-2, 3-8, 3-9): `ICompetitionService`/`CompetitionService` — create (name/date/creator required; club **xor** society host; referenced entities validated), list with host/status filters, get with entries+rounds, status transitions (Completed/Cancelled are terminal), managed entry add/withdraw, and self-registration status/register/withdraw (linked player only; open events accept anyone, members-only events require host membership; `HandicapAtEntry` snapshot; duplicate-proof; linked rounds must be unlinked before withdrawal). Round assign/unassign is owner-or-admin: non-admins must have entered and the round must include their linked player; admin backfill may still assign Completed competitions and auto-enters all round players. `RoundType` remains untouched. WHO may call create/manage is enforced at controller/page per §12.5 3.5: global admin, society Owner/Admin, or a global-admin-granted `ClubMembership.Admin` for that club. Ad-hoc competitions remain global-admin only.

API implemented (WORKLOG 3-4, 3-8, 3-9): `CompetitionsController` (`/api/competitions`) — list (club/society/status filters), detail with entries + linked round ids, create/update/status (gated by global Admin, the host society's Owner/Admin, or the host club's granted Admin; ad-hoc remains global-Admin-only), manager entry add/withdraw, and additive self-service `GET|POST|DELETE /{id}/registration` endpoints. Legacy entry endpoints route non-manager calls through the same linked-player eligibility rules for older mobile clients. Round assign/unassign uses `POST|DELETE …/rounds/{roundId}` with ownership and prior-entry enforcement in the service. API JWTs carry role claims so `User.IsInRole("Admin")` works under ApiAuth — tokens issued before this lack roles until next sign-in. DTOs in `Core/Models/Api/CompetitionDtos.cs` (enums serialised as strings per mobile conventions).

Round linking implemented (WORKLOG 3-6, refined in 3-9 and 3-11): the round-recording setup step and the round-details link control offer competitions to link — admins see every non-terminal competition, other users the non-terminal competitions their linked player has entered (`GetCompetitionsForPlayerAsync`); a hint appears when there are none. The chosen competition is set on the round at creation (both the RecordRound page and the RoundWorkflowService/live path via `RoundWorkflowSession.CompetitionId`), or linked/unlinked later from round details (`AssignRoundAsync`/`UnassignRoundAsync`, ownership-checked). Service enforcement prevents non-admin API callers linking before entry or after completion. **Assigning a round auto-enters the round's other players** (idempotent; handicap + tee snapshotted; admin backfill works on Completed competitions so 3-10 can reuse it). **Deleting a competition** (`DeleteCompetitionAsync`, manager-gated, web + `DELETE /api/competitions/{id}`) unlinks rounds and removes entries but never touches the rounds themselves — safe for test/beta cleanup. `RoundType` remains untouched throughout.

Results implemented (WORKLOG 3-5): `CompetitionService.ComputeResultsAsync` persists per-entry gross/net/Stableford and competition-style positions (ties share, e.g. 1,2,2,4; Stableford ranks by points, other formats by net) from each player's linked round — course handicap from `HandicapAtEntry` + the tee's rating/slope (entry tee → round tee → course default), scratch when data is missing; unlinked entries stay unranked; idempotent. Web UI (WORKLOG 3-5, 3-8, 3-9): `/competitions` list (status and society/club query filters, Open/Members-only labels; create dialog offers global admins every host and entity managers only their societies/clubs), `/competitions/{id}` detail (self-entry eligibility/register/withdraw card, distinct manager-only player entry tools, entries/results table with medals, and manager status actions — Complete auto-calculates results), Competitions in the nav Directory group and buttons on society/club detail. Global admins grant or revoke club-admin access from `/admin/users`; grants may create a membership and revocation preserves it as Member. Match Play v1 ranks by net like Medal (bracket play is future work).

Mobile UI implemented (WORKLOG 3-7, 3-8, 3-9): explicit `[JsonPropertyName]` DTO mirrors and `ICompetitionApiService` cover the additive competition API; custom routes provide global and society/club-filtered lists, a role-gated create dialog, detail, self-registration status/actions, entries/results, manager status/delete actions, and owned-round link/unlink flow. Competition cards identify Open versus Members-only entry; the detail page keeps self-entry separate from organizer player management and only offers ordinary users round linking after they have entered. Home, society, and club pages link into those routes. Society roles and club memberships shape host options and management controls (the API remains the authority). Round list responses add `CreatedByApplicationUserId` and `CompetitionId` so the client can distinguish owned/linkable rounds without changing existing fields. Completing through the API computes results before the mobile detail reloads; list queries load entries so summary counts are accurate.
...
- **Clubs are real-world shared entities** (pre-loaded from reference data; many members). A user must **not** self-claim ownership of a club. Club-admin rights are **granted by a global admin** as `ClubMembership.Admin` (a lightweight "claim + verify" flow may come later); revocation demotes the membership to Member rather than deleting it.
...
1. **Beta foundation (implemented in 3-1 through 3-4):** schema and service/API in place; initial competition creation + round assignment restricted to **global admin** so the feature could be exercised safely on demo data. Friendly/Competitive untouched.
2. **Society competitions (implemented in 3-4/3-5):** society Owner/Admin create + manage their society's competitions (uses existing membership roles).
3. **Club competitions (implemented in 3-8):** global admin grants club-admin; club admins create and manage their club competitions. Optional club claim+verify flow remains future work.
4. **Player participation (implemented in 3-9):** linked players register themselves for open competitions, or members-only competitions hosted by one of their clubs/societies, then assign one of their own rounds; results are computed per scoring format.
...
Steps 1–2 are implemented as pure functions in `GolfTrackerApp.Core/Services/WhsCalculator.cs` (`ComputeAdjustedGrossScore`, `ComputeDifferential`, `ComputeIndex` — index capped at 54.0, plus handicaps negative, half-away-from-zero rounding). Steps 3–4 are implemented by `HandicapService.OnRoundCompletedAsync` (idempotent: differentials upserted per player+round, `HandicapRecord` stored only on index change, `IsUsedInCalculation` flags maintained, `Player.Handicap` refreshed when `PrimaryHandicapSource` is Personal). It is invoked from **both** completion paths — `RoundService.UpdateRoundAsync` (status transition to Completed) and `ScoreService.SaveScorecardAsync` (completes directly) — and from `ScoreService.UpdateRoundScoresAsync` (the mobile score-edit path, so edited scorecards never leave stale differentials). All call sites are wrapped so a handicap failure never fails the score/round save.

When a round has no explicit tee selection (`RoundPlayer.TeeSetId` and the scores' `TeeSetId` are null — true for all historical/backfilled rounds), the engine resolves the course **default tee** — "Yellow", else the lowest-sort-order tee — so those rounds still qualify (WORKLOG 2-12). No round data is mutated; the resolved tee's rating/slope are snapshotted on the differential.
