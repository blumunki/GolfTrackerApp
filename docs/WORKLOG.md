...
| 2-18 | Set explicit decimal precision (HasPrecision) on handicap/scoring decimals + dual migration | 2 | Available | | 2026-06-27 | EF model-validation warnings on SqlServer: HandicapRecord.HandicapIndex, ScoringDifferential.CourseRating/Differential, TeeSet.CourseRating, CompetitionEntry.HandicapAtEntry (default decimal(18,2)). Not the prod outage (values are ≤1dp so no truncation now) but declare precision to silence warnings + be safe. Needs dual migration — do AFTER prod is stable |
...
| 3-4 | CompetitionsController (/api/competitions, JWT) — create/manage gated to global admin + society Owner/Admin (interim) | 3 | Available | | 2026-06-27 | Unblocked by 3-2. Admin check: User.IsInRole; society role via SocietyMembership. ARCHITECTURE §12.5 3.5 |
