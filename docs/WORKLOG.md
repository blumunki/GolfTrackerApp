...
| 2-13 | "My Golfing Profile" restructure (web): parameterized profile page housing stats + handicap, for own player AND managed players | 2 | Available | | 2026-06-12 | Engine already computes per-player (incl. managed); API already owner-or-self. UI/nav only — replaces My Stats + My Handicap nav entries. Fold into the P-1 IA epic — agree the overall structure first |
...
| 0-14 | Admin "Database Migrations" page: view applied/pending migrations + apply-on-demand button | 0 | In Progress | Claude | 2026-06-15 | The deliberate "apply when compute is back" lever (no redeploy). Needs a thin IDatabaseMigrationService (GetApplied/GetPending/MigrateAsync) + tests |
...
| 2-15 | Handicap dashboard: explain where the index comes from (value clarity) | 2 | Available | | 2026-06-15 | User feedback: index value isn't self-explanatory. Add a plain-English "how this is calculated" panel (avg of lowest 8 of last 20 differentials, etc.), make counting differentials + window clearer. NEEDS MANUAL REVIEW (is it clearer?) |
| P-1 | Navigation & information-architecture consolidation (epic — needs design direction) | polish | Available | | 2026-06-15 | Admin (11+ tiles) and non-admin pages sprawl. Cover: player-profile hub absorbing My Stats + My Handicap (supersedes 2-13); group nav by purpose; make admin tools discoverable (e.g. handicap-backfill tile exists but is buried among 11; migrations 0-14). Propose IA before building. NEEDS USER INPUT on grouping |
