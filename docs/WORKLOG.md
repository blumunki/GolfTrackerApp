...
| 2-8 | Recalculate differentials when scores are edited via RoundsController.UpdateRoundScores (move edit into a service + fire OnRoundCompletedAsync) | 2 | In Progress | Claude | 2026-06-12 | Found in 2-3: mobile score edits leave differentials stale; controller edits scores directly (thin-controller violation) |
| 2-9 | Manual club handicap entry UI (web) + primary-handicap selector | 2 | Available | | 2026-06-11 | Remaining 4b web UI; service CRUD + API exist (2-5). Selector updates Player.PrimaryHandicapSource and refreshes Player.Handicap |
