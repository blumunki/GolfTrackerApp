...
| 3-7 | Mobile competition models + API service + pages | 3 | In Progress | Codex | 2026-07-19 | Adding mobile DTOs, API client, custom routes, and competition list/detail pages |
...
| 3-10 | Competitions data sync (admin Data Migration) + retro-link historical Competitive rounds | 3 | Blocked | | 2026-06-27 | Rollout stage 5 (ARCHITECTURE §12.5 3.5). Deferred. Additive links only; no rewrite of historical RoundType. Design note: AssignRoundAsync now ensures entries for the round's players (works for Completed comps too) — backfill = create competition (Completed) + assign rounds; entries/results follow. Likely Competitions.csv (Name, Host, Course, Date, Format) + per-round links |
