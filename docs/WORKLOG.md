...
| 0-13 | Resilient startup migration: Database:MigrateOnStartup flag + graceful degradation (app starts when prod DB unavailable instead of crash-looping) | 0 | In Progress | Claude | 2026-06-15 | Program.cs currently rethrows on any DB init error in prod → crash-loop. Needed so 38 pending commits can deploy while prod DB is out of compute |
| 0-14 | Admin "Database Migrations" page: view applied/pending migrations + apply-on-demand button | 0 | Available | | 2026-06-15 | The deliberate "apply when compute is back" lever (no redeploy). Needs a thin IDatabaseMigrationService (GetApplied/GetPending/MigrateAsync) + tests |
| 2-14 | TeeSets.csv importer: tolerate "N/A"/non-numeric CourseRating & SlopeRating cells (map to null) | 2 | Available | | 2026-06-15 | Defect in 2-10: SlopeRating int?/CourseRating decimal? throw TypeConverterException on "N/A" → whole sync FATAL-errors at first N/A row. Add lenient converter or pre-clean. 7 N/A cells across Chalgrave/Belton Woodside/Tilsworth |
