...
| P-1a | Player Profile hub (tabs: Overview/Stats/Handicap/Rounds): consolidate stats + handicap + recent rounds per-player (self + managed); replaces My Stats + My Handicap nav | polish | In Progress | Claude | 2026-06-15 | Nav confirmed. Building: HandicapPanel (reusable) + 2-16 UI first, then the tabbed hub. PlayerReport already param'd by PlayerId. docs/NAVIGATION-IA.md §3 |
