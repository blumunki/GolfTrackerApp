...
| 3-11 | Round-link fixes from testing: link control discoverability (admins see all open comps, hint when none), auto-enter round players on assign, safe competition delete (unlinks rounds, never deletes them) | 3 | In Progress | Claude | 2026-06-27 | User testing found: link UI invisible without a prior entry; no delete path for test comps. Delete = admin/manager, unlink rounds + cascade entries, rounds untouched |
