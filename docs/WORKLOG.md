...
| 2-18 | Set explicit decimal precision (HasPrecision) on handicap/scoring decimals + dual migration | 2 | In Progress | Claude | 2026-06-27 | EF model-validation warnings on SqlServer: HandicapRecord.HandicapIndex, ScoringDifferential.CourseRating/Differential, TeeSet.CourseRating, CompetitionEntry.HandicapAtEntry (default decimal(18,2)). Prod now stable → safe to do. decimal(5,2) — superset of existing data, no value rounding on ALTER |
