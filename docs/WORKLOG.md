...
| P-1a | Player Profile hub (tabs: Overview/Stats/Handicap/Rounds): consolidate stats + handicap + recent rounds per-player (self + managed); replaces My Stats + My Handicap nav | polish | Done | Claude | 2026-06-15 | /players/{id}/profile + /profile; tabs reuse PlayerReport + HandicapPanel; per-player incl. managed. NEEDS manual review |
| P-1b | Player-facing nav: frequency-first — Record Round as primary CTA, flat everyday links, single collapsible Directory group | polish | Done | Claude | 2026-06-15 | Record Round CTA, My Profile→hub, Directory group; My Stats/My Handicap removed. App-bar CTA + mobile parity = follow-up polish. NEEDS manual review |
...
| P-2 | Nav polish: Record Round in the top app bar (web, one tap everywhere) | polish | Done | Claude | 2026-06-15 | App-bar CTA in MainLayout (full button on sm+, icon on xs); Hello-name hidden < md to make room. Build green |
| P-2m | Mobile bottom-nav parity: Record Round as the centre action | polish | Done | Codex | 2026-07-11 | Raised centre Record action preserves all shortcuts; Android build/deploy + authenticated emulator visual/tap-through verified; 213 tests green |
| P-3 | Profile/handicap/nav review fixes | polish | Done | Claude | 2026-06-15 | (1) Overview = player home (cards + recent rounds); (2) Stats embedded mode hides duplicate chrome; (3) denominator = 18-hole rounds (20), 9-hole summarised; (4) Club-Course labels; (5) admin side-nav grouped; (6) tiles open profiles; (7) connected tiles show handicap. 180 tests green. NEEDS manual review |
...
| 2-18 | Set explicit decimal precision (HasPrecision) on handicap/scoring decimals + dual migration | 2 | Done | Claude | 2026-06-27 | ConfigureConventions → decimal(5,2) model-wide (covers future decimals too); 5 AlterColumns on SqlServer, snapshot-sync on SQLite; no stored-value changes; 205 tests green. Applies to prod on next deploy |
...
| 3-2 | CompetitionService (create, list, get, status, link round, entry CRUD) + tests | 3 | Done | Claude | 2026-06-27 | Club-xor-society host validation; terminal-status guard; HandicapAtEntry snapshot; owner-or-admin round linking (RoundType untouched). 9 tests; 205 green |
| 3-3 | ScoringCalculator — pure scoring formats (Medal net, Stableford points, Match Play result) + TDD tests | 3 | Done | Claude | 2026-06-15 | ScoringCalculator (Medal/Stableford/Match Play) reusing StrokesReceivedOnHole; 14 tests; 196 total green. DB orchestration deferred to 3-2/results |
| 3-4 | CompetitionsController (/api/competitions, JWT) — create/manage gated to global admin + society Owner/Admin (interim) | 3 | Done | Claude | 2026-06-27 | Full surface (list/detail/create/update/status/entries/round-linking) + interim gates. JWTs now carry role claims (older tokens lack roles until re-login). 205 tests green |
| 3-5 | Web Competitions pages (List/Detail/Create/Results) + links from Society pages + nav | 3 | Done | Claude | 2026-06-27 | ComputeResultsAsync + IsHostManagerAsync in service (4 tests); /competitions + /competitions/{id} + create dialog + nav/society links. Complete auto-calculates results. 209 tests green. NEEDS manual click-through (create→enter→link round→complete→results) |
| 3-6 | Round recording: competition selector in setup; assign one of the player's own rounds to a competition they've entered (web) | 3 | Done | Claude | 2026-06-27 | Selector in RecordRound setup (+ workflow session for live path); link/unlink on RoundDetails (owner/admin); GetCompetitionsForPlayerAsync tested. 210 tests green. Core web loop complete — NEEDS full manual pass (create→enter→record+link→complete→results) |
| 3-7 | Mobile competition models + API service + pages | 3 | Done | Codex | 2026-07-19 | DTOs/client + custom list/society/detail routes; create, entries/results, status/delete and round link/unlink UI. Additive round ownership/competition fields; API completion calculates results. Android emulator end-to-end verified; 214 tests green |
| 3-8 | Club-admin model: global admin grants ClubMembership.Admin; club admins create club competitions | 3 | Done | Codex | 2026-07-19 | Global-admin grant/revoke UI in User Management; service + API host authorization; web/mobile club-filtered create/manage flows; clubs remain non-self-claimable. 218 tests green; web + Android builds clean |
| 3-9 | Public competition registration: users register for open competitions (and assign rounds) | 3 | Available | | 2026-07-19 | Rollout stage 4 (ARCHITECTURE §12.5 3.5); unblocked by completed club-admin rollout (3-8) |
| 3-10 | Competitions data sync (admin Data Migration) + retro-link historical Competitive rounds | 3 | Blocked | | 2026-07-19 | Rollout stage 5 (ARCHITECTURE §12.5 3.5), blocked by 3-9. Additive links only; no rewrite of historical RoundType. Design note: AssignRoundAsync now ensures entries for the round's players (works for Completed comps too) — backfill = create competition (Completed) + assign rounds; entries/results follow. Likely Competitions.csv (Name, Host, Course, Date, Format) + per-round links |
| 3-11 | Round-link fixes from testing: link control discoverability (admins see all open comps, hint when none), auto-enter round players on assign, safe competition delete (unlinks rounds, never deletes them) | 3 | Done | Claude | 2026-06-27 | AssignRound auto-enters round players (snapshot; works on Completed comps → 3-10 building block); Delete on service+web+API (rounds survive, tested); link UI always visible to owner/admin with hint. 213 tests green. NEEDS manual re-test of the link flow |
